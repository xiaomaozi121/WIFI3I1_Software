...
void moni_spi_Init( void )
{
  ...
  RCC_AHBPeriphClockCmd(GPIO_SPI_CLK, ENABLE);
  //GPIO_Init(GPIO_SPI_PORT,(GPIO_SPI_CS|GPIO_SPI_WR|GPIO_SPI_RD|GPIO_SPI_DATA),GPIO_MODE_OUT_PP_HIGH_FAST);
  GPIO_InitStructure.GPIO_Pin = (GPIO_SPI_CS|GPIO_SPI_WR|GPIO_SPI_RD|GPIO_SPI_DATA);
  ...
  GPIO_SetBits(GPIO_SPI_PORT, GPIO_SPI_CS);
  GPIO_SetBits(GPIO_SPI_PORT, GPIO_SPI_WR);
  GPIO_SetBits(GPIO_SPI_PORT, GPIO_SPI_RD);
  GPIO_SetBits(GPIO_SPI_PORT, GPIO_SPI_DATA);
}
...
static void moni_spi_Write_High (void)
{
  ...
  GPIO_ResetBits(GPIO_SPI_PORT,GPIO_SPI_WR);
  delay_us(1);
...
static void moni_spi_Write_Low (void)
{
  ...
  GPIO_ResetBits(GPIO_SPI_PORT,GPIO_SPI_DATA);
  delay_us(1);
  //GPIO_WriteHigh(GPIO_SPI_PORT,GPIO_SPI_WR);
  GPIO_SetBits(GPIO_SPI_PORT, GPIO_SPI_WR);
  delay_us(1);
...
static void LCD_Write( uint8_t Addr , uint8_t Data )
{
  ...
  uint8_t addr1 = Addr;
  //GPIO_WriteLow(GPIO_SPI_PORT,GPIO_SPI_CS);//LCD使能
  GPIO_ResetBits(GPIO_SPI_PORT,GPIO_SPI_CS);
  LCD_COM_Write();
	...
	void LCD_DisplayNum(unsigned char z,unsigned char n)
{
  ...
  LCD_Write(13,LED_L[n]);    // 19LED_L[n]
	break;	
