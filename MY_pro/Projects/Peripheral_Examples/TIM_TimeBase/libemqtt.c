...
//	};
	...
	uint8_t remainLen = 0;
	uint16_t offset = 0;
	uint8_t packet[sizeof(fixed_header)+sizeof(var_header1)+36];
	//uint8_t var_header1[12];
//	clientidlen = 9;	 //strlen(broker->clientid)
//	usernamelen = 11;	 //strlen(broker->username)
//	passwordlen = 9;	 //strlen(broker->password)
...
//	};
	var_header1[9] = flags;
	var_header1[10] = broker->alive>>8;
	var_header1[11] = broker->alive&0xFF, // Keep alive
   	// Fixed header
//    uint8_t fixedHeaderSize = 2;    // Default size = one byte Message Type + one byte Remaining Length
    //uint8_t remainLen = sizeof(var_header)+payload_len;
	remainLen = sizeof(var_header1)+payload_len;
//    if (remainLen > 127) {
...
//	uint8_t packet[sizeof(fixed_header)+sizeof(var_header)+payload_len];
	...
	memcpy(packet+offset, var_header1, sizeof(var_header1));
	offset += sizeof(var_header1);
...
//		return -1;
//	}
	send_packet(broker->socket_info, packet, sizeof(packet));
	return 1;
}
