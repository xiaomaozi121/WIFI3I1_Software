...
/**
  ...
  * @retval None
  */
uint8_t BHKS_ReveOneByte(USART_TypeDef* USARTx,uint8_t key)
{
...
void BHKS_SenduInt16_tString(USART_TypeDef* USARTx,uint16_t nInt)
{
	uint8_t nfloatString[6];
	if(nInt<10 && nInt>=1)
	{
		nfloatString[0]=nInt+0x30;
		nfloatString[1]='\0';
	}
	else if(nInt>=10 && nInt<100)
	{
		nfloatString[0]=nInt/10+0x30;
		nfloatString[1]=nInt%10+0x30;
		nfloatString[2]='\0';
	}
	else if(nInt>=100 && nInt<1000)
	{
		nfloatString[0]=nInt/100+0x30;
		nfloatString[1]=nInt%100/10+0x30;
		nfloatString[2]=nInt%10+0x30;
		nfloatString[3]='\0';
	}
	else if(nInt>=1000 && nInt<10000)
	{
		nfloatString[0]=nInt/1000+0x30;
		nfloatString[1]=nInt%1000/100+0x30;
		nfloatString[2]=nInt%100/10+0x30;
		nfloatString[3]=nInt%10+0x30;
		nfloatString[4]='\0';
	}
	else if(nInt>=10000)
	{
		nfloatString[0]=nInt/10000+0x30;
		nfloatString[1]=nInt%10000/1000+0x30;
		nfloatString[2]=nInt%1000/100+0x30;
		nfloatString[3]=nInt/100%10+0x30;
		nfloatString[4]=nInt%10+0x30;
		nfloatString[5]='\0';
	}
	BHKS_Serial_PutString(USARTx,nfloatString);
}
void BHKS_SendFloatString(USART_TypeDef* USARTx,float nfloat)//发送字符串 函数   应用指针 方法
{
  ...
  else if(nfloat>=0.01 && nfloat< 0.1)
  {
    nfloatString[0]='0';
		nfloatString[1]='.';
    nfloatString[2]='0';
    nfloatString[3]=(uint8_t)(nfloat*100)%10+0x30;
		nfloatString[4]='\0';
  }
	else if(nfloat>=0.1 && nfloat<1.0)
	{
		nfloatString[0]='0';
		nfloatString[1]='.';
    nfloatString[2]=(uint8_t)(nfloat*100)/10+0x30;
    nfloatString[3]=(uint8_t)(nfloat*100)%10+0x30;
		nfloatString[4]='\0';
	}
	else if(nfloat >=1.0 && nfloat<10.0)
	{
		nfloatString[0]=(uint8_t)((uint16_t)(nfloat*100)/100)+0x30;
		nfloatString[1]='.';
		nfloatString[2]=(uint8_t)((uint16_t)(nfloat*100)%100/10)+0x30;
		nfloatString[3]=(uint8_t)((uint16_t)(nfloat*100)%10)+0x30;
		nfloatString[4]='\0';
	}
  BHKS_Serial_PutString(USARTx,nfloatString);
}
