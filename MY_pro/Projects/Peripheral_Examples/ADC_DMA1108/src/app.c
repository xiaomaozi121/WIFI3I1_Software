 ...
 //@near static WORD PM_V_buf[30];
   ...
   PM2.5 ¥¶¿Ì≥Ã–Ú 5¥Œ≤…—˘∆Ωæ˘ À¢–¬ æ ˝
		...
		void PM_Process(void)
	{
		...
		u16 t;
		//sum_adc3 -= (min_adc3 + max_adc3);//»•µÙ◊Ó¥Û÷µ∫Õ◊Ó–°÷µ
   ...
   // min_adc3 = PM_V/3;
		...
		{
		  //PM_V_tmp =(unsigned long int) ((PM_average-parameters.para.A_PM)*parameters.para.K1_PM);
...
void E2P_Write(unsigned int addr, uint8_t* p_data, unsigned int len)
{
	u8 i;
...
void E2P_Read(unsigned int addr, uint8_t* p_data, unsigned int len)
{
		u8 i;
		FLASH_Unlock(FLASH_MEMTYPE_DATA);
