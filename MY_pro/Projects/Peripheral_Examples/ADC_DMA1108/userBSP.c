...
void TIM_Config(void)
{
 ...
 //PrescalerValue = (uint16_t) (SystemCoreClock  / 6000000) - 1;
  PrescalerValue = (uint16_t) (SystemCoreClock  / 2000) - 1;  //48m/24k =2k

  /* Time base configuration */
  //TIM_TimeBaseStructure.TIM_Period = 65535;
  TIM_TimeBaseStructure.TIM_Period = 2000-1;//4000-1;//2000-1;//1ms
...
void GPIO_Init_User(void)
 {
   int a ;
	...
	GPIO_Init(GPIO_BL_PORT,GPIO_BL,GPIO_MODE_OUT_PP_HIGH_FAST);
...
{
		#ifdef Beep_Active
	GPIO_WriteHigh(GPIO_Beep_PORT,GPIO_Beep); 
	 #else
	 BEEP_Cmd(ENABLE);
	 #endif
}
...
void Beep_Off(void)
{
	...
	#ifdef Beep_Active
	GPIO_WriteLow(GPIO_Beep_PORT,GPIO_Beep);  
	 #else
	 BEEP_Cmd(DISABLE);
	 //if PV
		...
		if(	PM_En_Flag )
    {
		...
		if(timer5_cnt ==100)           //秒 计数
		{
			...
			//DisFlag = SET; //没秒一次示数刷新
		 		...
		 		timer5_cnt++;
/*	#ifdef WIFI
	if((wifi_nRLD_FLG == SET)&&(GPIO_ReadInputPin(GPIO_Key4_PORT,GPIO_Key_SW4)== RESET))
	{
		GPIO_WriteHigh(GPIOD, wifi_nRLD);
		wifi_nRLD_FLG = RESET;
	}
	*/#endif
}
